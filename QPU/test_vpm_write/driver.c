...
struct memory_map {
    ...
    unsigned int uniforms[NUM_QPUS][1];     // the address of the
                                            // result buffer
    unsigned int msg[NUM_QPUS][2];
    unsigned int results[32][2];     // result buffer for the QPU to
                                            // write into
};
...
int main(int argc, char **argv)
{
    ...
    //Init results.
    for (int i=0; i < 32; i++) {
        arm_map->results[i][0] = 1000+i;
        arm_map->results[i][1] = 1000+i;
    }
